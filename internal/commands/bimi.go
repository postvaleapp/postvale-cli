...
func newBimiCommand() *cobra.Command {
	return &cobra.Command{
		Use:   "bimi <domain>",
		Short: "BIMI record + VMC + logo asset",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			result, err := client.CheckBimi(domain)
			if err != nil {
				return fmt.Errorf("bimi %s: %w", domain, err)
			}
			...
			if g.Quiet {
				...
				output.RenderBimi(cmd.OutOrStdout(), result)
			}
